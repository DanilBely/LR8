...
/*параметризированный тип. Особая роль параметризированных типов состоит в том, что они позволяют создавать классы, структуры, интерфейсы, методы и делегаты, 
 * в которых обрабатываемые данные указываются в виде параметра. 
...
namespace OOP_8
{
    ...
    public class Program
    {
        ...
        public class CollectionType<T> : IDo<T> where T : new()//т - тип данных имеющий открытый конструктор без параметров
        {
            public List<T> Set { get; set; }//вложили обобщенную колекцию
            public int k;
